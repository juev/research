...
**Уровень 2 — разработчик поверх чужой инфраструктуры.** Нужен аккаунт и способ аутентификации против публичных эндпоинтов. Хостов несколько: Relay (`bsky.network`), entryway (`bsky.social`), AppView с опциональной авторизацией (`api.bsky.app`) и публичная AppView без авторизации (`public.api.bsky.app`).[^api-hosts] Здесь `bsky.social` — тот же адрес, что в Уровне 1: entryway — это «входной» сервис Bluesky перед её PDS-кластером, он заводит аккаунты и раздаёт авторизацию, а не отдельный от PDS хост. Простейший способ получить сессию — метод `createSession` по handle и паролю приложения: в ответ приходят `accessJwt` (живёт минуты) и `refreshJwt` (живёт дольше).[^getstarted] Этого legacy-пути (app password) хватает для скриптов и ботов — читать ленту, публиковать посты, слушать данные, ничего не хостя; для приложений с интерфейсом правильный путь — OAuth (см. ниже). Публичные эндпоинты не бесконечны: у API есть свои rate limits, которые для боевого приложения приходится учитывать.
...
Важная оговорка про федерацию своего PDS: она открывалась в режиме раннего доступа с жёсткими лимитами — 10 аккаунтов на PDS, 1500 событий в час и 10 000 в сутки (пост от февраля 2024-го; ограничения вводились как временные предохранители от абуза).[^selfhost-fed] Это ранний снимок: числа с тех пор могли смягчиться, актуальные значения стоит смотреть в текущей документации. Но принцип остаётся: «поднять свой PDS» и «участвовать в сети на равных» — не одно и то же (подробнее в разделе про ограничения).
